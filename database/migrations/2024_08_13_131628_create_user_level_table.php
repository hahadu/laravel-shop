...
return new class extends Migration {
    ...
    public function up(): void
    {
        Schema::create($this->table_name, function (Blueprint $table) {
            ...
            $table->tinyInteger('is_forever',1)->default(0)->comment('是否永久');
            $table->integer('mer_id')->default(0)->comment('商户id');
            $table->tinyInteger('status',1)->default(0)->comment('0:禁止,1:正常');
            $table->char('mark',255)->default('')->comment('备注');
            $table->tinyInteger('remind',1)->default(0)->comment('是否已通知');
            $table->tinyInteger('is_del',1)->default(0)->comment('是否删除,0=未删除,1=删除');
