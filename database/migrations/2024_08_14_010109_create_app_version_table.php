...
return new class extends Migration {
    ...
    public function up(): void
    {
        Schema::create($this->table_name, function (Blueprint $table) {
            ...
            $table->tinyInteger('platform')->default(0)->comment('平台类型:1.安卓 2.IOS');
            $table->text('info')->nullable()->comment('升级信息');
            $table->char('url',255)->default('')->comment('下载链接');
            $table->tinyInteger('is_force')->default(1)->comment('是否强制升级');
            $table->tinyInteger('is_new')->default(0)->comment('是否最新');
