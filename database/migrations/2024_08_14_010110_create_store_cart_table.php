...
return new class extends Migration {
    ...
    public function up(): void
    {
        Schema::create($this->table_name, function (Blueprint $table) {
            ...
            $table->smallInteger('cart_num')->unsigned()->default(0)->comment('商品数量');
            $table->integer('add_time')->unsigned()->default(0)->comment('添加时间');
            $table->tinyInteger('is_pay')->default(0)->comment('0 = 未购买 1 = 已购买');
            $table->tinyInteger('is_del')->default(0)->comment('是否删除');
            $table->tinyInteger('is_new')->default(0)->comment('是否为立即购买');
            ...
            $table->tinyInteger('status')->default(1)->comment('购物车商品状态');
        });
