...
return new class extends Migration {
    ...
    public function up(): void
    {
        Schema::create($this->table_name, function (Blueprint $table) {
            ...
            $table->tinyInteger('is_tourist')->default(0)->comment('1=游客模式，0=非游客');
            $table->tinyInteger('time_node')->default(0)->comment('时间节点');
            $table->integer('add_time')->default(0)->comment('发送时间');
            $table->tinyInteger('type')->default(0)->comment('是否已读（0：否；1：是；）');
            $table->tinyInteger('remind')->default(0)->comment('是否提醒过');
