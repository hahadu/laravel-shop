...
return new class extends Migration {
    ...
    public function up(): void
    {
        Schema::create($this->table_name, function (Blueprint $table) {
            ...
            $table->char('headimgurl',255)->default('')->comment('用户头像');
            ...
            $table->char('remark',255)->default('')->comment('公众号运营者对粉丝的备注，公众号运营者可在微信公众平台用户管理界面对粉丝添加备注');
            $table->smallInteger('groupid')->index()->unsigned()->default(0)->comment('用户所在的分组ID（兼容旧的用户分组接口）');
            $table->char('tagid_list',255)->default('')->comment('用户被打上的标签ID列表');
            ...
            $table->tinyInteger('is_complete')->default(0)->comment('信息是否完善');
