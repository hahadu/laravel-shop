...
return new class extends Migration {
    ...
    public function up(): void
    {
        Schema::create($this->table_name, function (Blueprint $table) {
            ...
            $table->tinyInteger('is_cancel')->default(0)->comment('用户是否取消');
            $table->tinyInteger('is_pink_cancel')->default(0)->comment('是否拼团自动取消');
            $table->tinyInteger('is_del')->nullable()->default(0)->comment('取消申请');
            $table->integer('add_time')->default(0)->comment('申请退款时间');
            $table->tinyInteger('is_system_del')->nullable()->default(0)->comment('系统删除');
        });
