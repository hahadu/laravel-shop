...
return new class extends Migration {
    ...
    public function up(): void
    {
        Schema::create($this->table_name, function (Blueprint $table) {
            ...
            $table->tinyInteger('status')->index()->default(0)->comment('订单状态（-1 : 申请退款 -2 : 退货成功 0：待发货；1：待收货；2：已收货；3：待评价；-1：已退款）');
            $table->tinyInteger('is_stock_up')->default(0)->comment('是否备货中');
            $table->tinyInteger('refund_status')->unsigned()->default(0)->comment('0 未退款 1 申请中 2 已退款');
            $table->tinyInteger('refund_type')->default(0)->comment('退款申请类型');
            $table->char('refund_express',255)->default('')->comment('退货快递单号');
            $table->char('refund_express_name',255)->default('')->comment('退货快递名称');
            $table->char('refund_reason_wap_img',255)->default('')->comment('退款图片');
            ...
            $table->char('fictitious_content',255)->default('')->comment('虚拟发货内容');
            ...
            $table->char('remark',255)->default('')->comment('管理员备注');
            ...
            $table->tinyInteger('shipping_type')->default(1)->comment('配送方式 1=快递 ，2=门店自提');
            ...
            $table->tinyInteger('is_system_del')->default(0)->comment('后台是否删除');
            $table->char('channel_type',255)->default('')->comment('用户访问端标识');
            $table->char('province',255)->default('')->comment('用户省份');
            $table->char('express_dump',255)->default('')->comment('订单面单打印信息');
