...
return new class extends Migration {
    ...
    public function up(): void
    {
        Schema::create($this->table_name, function (Blueprint $table) {
            ...
            $table->char('title',255)->default('')->comment('通知标题');
            $table->char('content',255)->default('')->comment('通知内容');
            $table->char('data',255)->default('')->comment('站内信参数');
            $table->tinyInteger('look')->default(0)->comment('是否查看');
            $table->tinyInteger('type')->default(0)->comment('1:普通用户，2：管理员');
