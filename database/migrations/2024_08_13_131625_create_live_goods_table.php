...
return new class extends Migration {
    ...
    public function up(): void
    {
        Schema::create($this->table_name, function (Blueprint $table) {
            ...
            $table->tinyInteger('price_type',1)->default(1)->comment('价格类型（1:一口价，此时读price字段; 2:价格区间，此时price字段为左边界，price2字段为右边界; 3:折扣价，此时price字段为原价，price2字段为现价；）');
            ...
            $table->tinyInteger('audit_status',1)->default(0)->comment('审核状态（0：未审核，1：审核中，2:审核通过，3审核失败）');
            $table->tinyInteger('third_part_tag',1)->default(1)->comment('1、2：表示是为 API 添加商品，否则是直播控制台添加的商品');
            $table->smallint('sort')->default(0)->comment('排序');
            $table->tinyInteger('is_show',1)->default(1)->comment('是否显示');
