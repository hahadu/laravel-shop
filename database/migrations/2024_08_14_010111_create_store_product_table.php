...
return new class extends Migration {
    ...
    public function up(): void
    {
        Schema::create($this->table_name, function (Blueprint $table) {
            ...
            $table->char('image',255)->default('')->comment('商品图片');
            $table->char('recommend_image',255)->default('')->comment('推荐图');
            $table->char('slider_image',255)->default('')->comment('轮播图');
            $table->char('store_name',128)->default('')->comment('商品名称');
            $table->char('store_info',255)->default('')->comment('商品简介');
            $table->char('keyword',255)->default('')->comment('关键字');
            ...
            $table->smallInteger('sort')->index()->default(0)->comment('排序');
            $table->mediumInteger('sales')->index()->unsigned()->default(0)->comment('销量');
            $table->mediumInteger('stock')->unsigned()->default(0)->comment('库存');
            $table->tinyInteger('is_show')->index()->default(1)->comment('状态（0：未上架，1：上架）');
            $table->tinyInteger('is_hot')->index()->default(0)->comment('是否热卖');
            $table->tinyInteger('is_benefit')->index()->default(0)->comment('是否优惠');
            $table->tinyInteger('is_best')->index()->default(0)->comment('是否精品');
            $table->tinyInteger('is_new')->index()->default(0)->comment('是否新品');
            $table->tinyInteger('is_virtual')->default(0)->comment('商品是否是虚拟商品');
            $table->tinyInteger('virtual_type')->default(0)->comment('虚拟商品类型');
            ...
            $table->tinyInteger('is_good')->default(0)->comment('是否优品推荐');
            $table->tinyInteger('is_sub')->default(0)->comment('是否单独分佣');
            $table->tinyInteger('is_vip')->default(0)->comment('是否开启会员价格');
            $table->mediumInteger('ficti')->default(0)->comment('虚拟销量');
            ...
            $table->char('video_link',255)->default('')->comment('主图视频链接');
            $table->integer('temp_id')->default(1)->comment('运费模板ID');
            $table->tinyInteger('spec_type')->default(0)->comment('规格 0单 1多');
            ...
            $table->tinyInteger('vip_product')->default(0)->comment('是否会员专属商品');
            $table->tinyInteger('presale')->default(0)->comment('是否预售商品');
            ...
            $table->tinyInteger('is_limit')->default(0)->comment('是否开启限购');
            $table->tinyInteger('limit_type')->default(0)->comment('限购类型1单次限购2永久限购');
