...
return new class extends Migration {
    ...
    public function up(): void
    {
        Schema::create($this->table_name, function (Blueprint $table) {
            ...
            $table->smallInteger('one_brokerage')->default(0)->comment('一级分拥上浮比例');
            $table->decimal('one_brokerage_percent',10,2)->default(0.00)->comment('一级分佣比例');
            $table->smallInteger('two_brokerage')->default(0)->comment('二级分拥上浮比例');
            $table->decimal('two_brokerage_percent',10,2)->default(0.00)->comment('二级分佣比例');
            $table->smallInteger('grade')->default(0)->comment('等级');
