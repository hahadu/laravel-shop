...
return new class extends Migration {
    ...
    public function up(): void
    {
        Schema::create($this->table_name, function (Blueprint $table) {
            ...
            $table->tinyInteger('product_score',1)->index()->default(0)->comment('商品分数');
            $table->tinyInteger('service_score',1)->index()->default(0)->comment('服务分数');
            $table->char('comment',512)->default('')->comment('评论内容');
            $table->text('pics')->nullable()->comment('评论图片');
            $table->integer('add_time')->index()->default(0)->comment('评论时间');
            $table->char('merchant_reply_content',300)->default('')->comment('管理员回复内容');
            ...
            $table->tinyInteger('is_reply',1)->default(0)->comment('0未回复1已回复');
            ...
            $table->tinyInteger('status',1)->default(0)->comment('评论状态');
        });
