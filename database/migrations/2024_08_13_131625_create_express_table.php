...
return new class extends Migration {
    ...
    public function up(): void
    {
        Schema::create($this->table_name, function (Blueprint $table) {
            ...
            $table->tinyInteger('partner_id',1)->default(0)->comment('是否需要月结账号');
            $table->tinyInteger('partner_key',1)->default(0)->comment('是否需要月结密码');
            $table->tinyInteger('net',1)->default(0)->comment('是否需要取件网店');
            $table->tinyInteger('check_man',1)->default(0)->comment('是否填写电子面单承载快递员名');
            $table->tinyInteger('partner_name',1)->default(0)->comment('是否填写电子面单客户账户名称');
            $table->tinyInteger('is_code',1)->default(0)->comment('是否填写电子面单承载编号');
            ...
            $table->tinyInteger('is_show',1)->index()->default(0)->comment('是否显示');
            $table->tinyInteger('status',1)->default(0)->comment('是否可用');
        });
