...
return new class extends Migration {
    ...
    public function up(): void
    {
        Schema::create($this->table_name, function (Blueprint $table) {
            ...
            $table->tinyInteger('type',1)->default(1)->comment('奖品类型1：未中奖2：积分3:余额4：红包5:优惠券6：站内商品7：等级经验8：用户等级 9：svip天数');
            $table->tinyInteger('is_receive',1)->default(0)->comment('是否领取');
            $table->integer('receive_time')->default(0)->comment('领取时间');
            $table->text('receive_info')->nullable()->comment('收获地址、备注等');
            $table->tinyInteger('is_deliver',1)->default(0)->comment('是否发货');
