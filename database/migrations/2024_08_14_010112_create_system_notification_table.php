...
return new class extends Migration {
    ...
    public function up(): void
    {
        Schema::create($this->table_name, function (Blueprint $table) {
            ...
            $table->tinyInteger('is_system')->default(0)->comment('站内信（0：不存在，1：开启，2：关闭）');
            $table->char('system_title',255)->default('')->comment('站内信标题');
            $table->char('system_text',255)->default('')->comment('系统消息id');
            $table->tinyInteger('is_wechat')->default(0)->comment('公众号模板消息（0：不存在，1：开启，2：关闭）');
            ...
            $table->tinyInteger('is_routine')->default(0)->comment('小程序订阅消息（0：不存在，1：开启，2：关闭）');
            ...
            $table->tinyInteger('is_sms')->default(0)->comment('发送短信（0：不存在，1：开启，2：关闭）');
            $table->char('sms_id',32)->default('')->comment('短信id');
            $table->char('sms_text',255)->default('')->comment('短信模版内容');
            $table->tinyInteger('is_ent_wechat')->default(0)->comment('企业微信群通知（0：不存在，1：开启，2：关闭）');
            $table->char('ent_wechat_text',255)->default('')->comment('企业微信消息');
            $table->char('url',255)->default('')->comment('群机器人链接');
            $table->tinyInteger('is_app')->default(0)->comment('APP推送（0：不存在，1：开启，2：关闭）');
            $table->integer('app_id')->default(0)->comment('app推送id');
            $table->char('variable',255)->default('')->comment('变量');
            $table->tinyInteger('type')->default(1)->comment('类型（1：用户，2：管理员）');
            $table->integer('add_time')->default(0)->comment('添加时间');
            $table->char('custom_trigger',255)->default('')->comment('自定义消息触发位置');
            $table->char('custom_variable',255)->default('')->comment('自定义消息变量');
        });
